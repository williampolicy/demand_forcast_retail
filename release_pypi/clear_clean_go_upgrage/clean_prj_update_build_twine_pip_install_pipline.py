...
def uninstall_old_version():
    print("Uninstalling old version of the package...")
    subprocess.check_call(['pip3', 'uninstall', '-y', 'kangforecast'])

def clear_pip_cache():
    print("Clearing pip cache...")
    subprocess.check_call(['pip3', 'cache', 'purge'])

def install_new_version(new_version):
    print(f"Attempting to install new version: {new_version}")
    max_attempts = 3
    delay_between_attempts = 3
    for attempt in range(1, max_attempts + 1):
        try:
            subprocess.check_call(['pip3', 'install', '--no-cache-dir', f'kangforecast=={new_version}'])
...
def main():
    parser = argparse.ArgumentParser(description="""
        This script performs several steps to update, build, upload and install a new version of the 'kangforecast' package. Here are the detailed steps:
    ...
    clear_pip_cache()
    install_new_version(new_version)
    print("All done!")
